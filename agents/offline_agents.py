...
import phyre
import neural_agent
import neural_agent_contrastive
from psim import ParallelPhyreSimulator

from extended_evaluation import DetailedEvaluator
import nets
...
class DQNAgent(AgentWithSimulationCache):
    ...
    def add_parser_arguments(cls, parser: 'argparse.ArgumentParser') -> None:
        parser = parser.add_argument_group('%s params' % cls.__name__)
        parser.add_argument('--dqn-tensorboard-dir', type=str, default="")
        ...
        parser.add_argument('--dqn-network-type',
                            choices=('resnet18', 'resnet50', 'simple', 'framewise_contrastive'),
                            default='resnet18',
        ...
        parser.add_argument('--dqn-checkpoint-dir', type=str, default="")

        parser.add_argument('--dqn-n-samples-per-task', type=int, default=0)

        parser.add_argument('--dqn-use-sample-distance-aux-loss',
                            type=int,
                            default=0)
        parser.add_argument('--dqn-lhp-distance-cutoff',
                            type=float,
                            default=None)
        parser.add_argument('--dqn-lhp-regression-type',
                            choices=("mse", "binned"),
                            default="mse")
        parser.add_argument('--dqn-lhp-repr-merging-method',
                            choices=("outer", "mul", "concat"))
        parser.add_argument('--dqn-lhp-n-regression-bins', type=int, default=0)
        parser.add_argument('--dqn-lhp-time-slice', type=int, default=0)
        parser.add_argument('--dqn-lhp-embedding-dim', type=int, default=256)
        parser.add_argument('--dqn-lhp-embeddor-type',
                            choices=("linear", "mlp", "mlp_2hidden", "none"),
                            default="linear")
        parser.add_argument('--dqn-lhp-weight', type=float, default=1.0)
        parser.add_argument('--dqn-debug', type=int, default=0)
    ...
    def _train_with_cache(cls, cache, task_ids, tier, dqn_load_from,
        ...
        if dqn_load_from is not None:
            ...
            aux_loss_hp = {
                arg[4:]: value
                for arg, value in train_kwargs.items()
                if arg.startswith("lhp_")
            }
            for key in list(train_kwargs.keys()):
                if key.startswith("lhp_"):
                    del train_kwargs[key]
            if aux_loss_hp:
                train_kwargs["aux_loss_hyperparams"] = aux_loss_hp
...
def get_latest_eval_checkpoint(output_dir):
    known_checkpoints = sorted(glob.glob(os.path.join(output_dir, 'eval.*')))
    if known_checkpoints:
        return known_checkpoints[-1]
    else:
        return None

class DQNAgentContrastive(AgentWithSimulationCache):
    ...
    @classmethod
    def name(cls):
        return 'dqn_self_supervised'

    @classmethod
    def add_parser_arguments(cls, parser: 'argparse.ArgumentParser') -> None:
        parser = parser.add_argument_group('%s params' % cls.__name__)
        parser.add_argument('--dqn-framewise-contrastive-n-frames', type=int,
                help='Length of video segment to be used in framewise_contrastive model')
        parser.add_argument('--dqn-lhp-additional-negatives', type=int, default=0)
        parser.add_argument('--dqn-lhp-n-negatives', type=int, default=-1)
        parser.add_argument('--dqn-lhp-temperature', type=float, default=1.0)
    ...
    def _train_with_cache(cls, cache, task_ids, tier, dqn_load_from,
                          ...
                          **kwargs):
        if dqn_load_from is not None:
            model = neural_agent_contrastive.load_agent_from_folder(dqn_load_from)
        else:
            train_kwargs, _ = cls._extract_dqn_flags(**kwargs)
            aux_loss_hp = {
                arg[4:]: value
            ...
            if aux_loss_hp:
                train_kwargs["aux_loss_hyperparams"] = aux_loss_hp
            model = neural_agent_contrastive.train(output_dir,
                                       tier,
                                       task_ids,
                                       cache=cache,
                                       max_train_actions=max_train_actions,
    ...
    def eval(cls, state, task_ids, max_attempts_per_task, tier, dqn_rank_size,
             **kwargs):
        model = state['model']
        train_kwargs, eval_kwargs = cls._extract_dqn_flags(**kwargs)
        ...
        actions = state['cache'].action_array[:dqn_rank_size]
        return cls.real_eval(state['cache'], model, actions, task_ids, tier,
                        max_attempts_per_task, checkpoint_dir=train_kwargs["checkpoint_dir"],**eval_kwargs)
