...
def main_with_seed(eval_setup_name, fold_id, use_test_split,
    ...
    details_out_path = os.path.join(output_dir, 'detailed_results.csv')
    with open(out_path, 'w') as stream:
        json.dump(results, stream)
    with open(details_out_path, "w") as stream:
        stream.write(evaluation.dump_report())
